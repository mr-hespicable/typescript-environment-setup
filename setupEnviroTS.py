...
cdToPath = f'cd {path} &&'
os.system(f'{cdToPath} mkdir {name}') #creating directory
dir = f'cd {path}/{name} &&'
os.system(f'{dir} npm init -y && {dir} npm i typescript --save-dev && {dir} npm i @types/node && {dir} npx tsc && {dir} npx tsc --init') #setting up intial environment

with open(f'defaultTSCONFIG.txt', 'r') as firstfile, open(f'{path}/{name}/tsconfig.json', 'a') as secondfile:
    for line in firstfile:
      secondfile.write(line)
  
os.system(f'{dir} touch test.ts') #creating test file
